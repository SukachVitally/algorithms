...
class ParallelMergeSorter2 {

    private int[] array;

    public ParallelMergeSorter2(int[] array) {
        this.array = array;
    }
    ...
    public int[] sort() {
        ...
        return pool.invoke(new ParallelWorker(array));
    }

    /* worker that gets called recursively */
    private class ParallelWorker extends RecursiveTask<int[]> {

        private int[] items;

        public ParallelWorker(int[] items) {
            this.items = items;
        }
        ...
        protected int[] compute() {
            ...
            int[] tempLeftArray = Arrays.copyOfRange(items, 0, mid);
            int[] tempRightArray = Arrays.copyOfRange(items, mid, items.length);

            int[] leftArray= new ParallelWorker(tempLeftArray).compute();
            int[] rightArray = new ParallelWorker(tempRightArray).compute();

            var res = new int[items.length];
            var ind1 = 0;
            var ind2 = 0;
            for(var i = 0; i < items.length; i++) {
                var var1 = leftArray.length > ind1 ? leftArray[ind1] : null;
                var var2 = rightArray.length > ind2 ? rightArray[ind2] : null;
...
public class MergeSortingTest {
    ...
    public static void main(String[] args) {
        ...
        final int[] input = generateRandomArray(1000_000);
...
//        ParallelMergeSorter2 pms = new ParallelMergeSorter2(Arrays.copyOf(input, input.length));
//        int[] parallelResult = pms.sort();
//        double parallelTime = 0;
//        for(int i=0; i<NUM_EVAL_RUNS; i++) {
//            pms = new ParallelMergeSorter2(Arrays.copyOf(input, input.length));
//            long start = System.currentTimeMillis();
//            pms.sort();
